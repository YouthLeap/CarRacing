...
public class DiscountGiftControllor : UIBoxBase {
	...
	public override void Show ()
	{
		...
		PayUIConfigurator.PayUIConfig(PayType.DiscountGift, textHint1, textHint2, textDesc1, textDesc2, textPayBt, imageCloseBt, goBuyButton.GetComponent<BoxCollider>(), SetCancelBt);
	...
	{
		base.Hide();
		UIManager.Instance.HideModule (UISceneModuleType.DiscountGift);
	...
	{
		AudioManger.Instance.PlaySound(AudioManger.SoundName.ButtonClick);
		PayBuildPayManage.Instance.Pay ((int)PayType.DiscountGift, GetGiftPayCallBack);
	...
	void GetGiftPayCallBack(string result)
	{
		...
		Hide();
		PlayerData.Instance.SetCTypeBagState (true);
