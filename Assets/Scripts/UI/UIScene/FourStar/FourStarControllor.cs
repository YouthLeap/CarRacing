...
public class FourStarControllor : UIBoxBase {
	...
	{
		titleText.text = PayJsonData.Instance.GetGiftTitle(PayType.RewardsGift4Stars);
		string[] IconArr  = PayJsonData.Instance.GetGiftItemsIconArr(PayType.RewardsGift4Stars);
		int[]    CountArr = PayJsonData.Instance.GetGiftItemsCountsArr(PayType.RewardsGift4Stars);
	...
	public override void Show ()
	{
		...
		PayUIConfigurator.PayUIConfig(PayType.RewardsGift4Stars, textHint1, textHint2, textDesc1, textDesc2, textPayBt, imageCloseBt, goBuyButton.GetComponent<BoxCollider>(), SetCancelBt);
	...
	void SetCancelBt(bool state)
	{
		...
		if(state)
		{
			...
			goCancelBtn.transform.localPosition = new Vector3(-88, -134, 0);
			goBuyButton.transform.localPosition = new Vector3(115, -134, 0);
		}
		else
		{
			goCancelBtn.SetActive(false);
			goBuyButton.transform.localPosition = new Vector3(90, -134, 0);
		}
	}
	...
	{
		base.Hide();
		UIManager.Instance.HideModule (UISceneModuleType.FourStar);
	}
	...
	{
		AudioManger.Instance.PlaySound(AudioManger.SoundName.ButtonClick);
		PayBuildPayManage.Instance.Pay((int)PayType.RewardsGift4Stars, BuyPassGiftCall);
	...
	void BuyPassGiftCall(string result)
	{
		...
		PlayerData.ItemType[] itemType = PayJsonData.Instance.GetGiftItemsTypeArr(PayType.RewardsGift4Stars);
		int[] count = PayJsonData.Instance.GetGiftItemsCountsArr(PayType.RewardsGift4Stars);
