...
public class ThreeStarControllor : UIBoxBase {
	...
	{
		titleText.text = PayJsonData.Instance.GetGiftTitle(PayType.RewardsGift3Stars);
		string[] IconArr  = PayJsonData.Instance.GetGiftItemsIconArr(PayType.RewardsGift3Stars);
		int[]    CountArr = PayJsonData.Instance.GetGiftItemsCountsArr(PayType.RewardsGift3Stars);
	...
	public override void Show ()
	{
		...
		PayUIConfigurator.PayUIConfig(PayType.RewardsGift3Stars, textHint1, textHint2, textDesc1, textDesc2, textPayBt, imageCloseBt, goBuyButton.GetComponent<BoxCollider>(), SetCancelBt);
	...
	void SetCancelBt(bool state)
	{
		...
		else
		{
			...
			goBuyButton.transform.localPosition = new Vector3(50, -136, 0);
		}
	}
	...
	{
		base.Hide();
		UIManager.Instance.HideModule (UISceneModuleType.ThreeStar);
	}
	...
	{
		AudioManger.Instance.PlaySound(AudioManger.SoundName.ButtonClick);
		PayBuildPayManage.Instance.Pay((int)PayType.RewardsGift3Stars, BuyPassGiftCall);
	...
	void BuyPassGiftCall(string result)
	{
		...
		PlayerData.ItemType[] itemType = PayJsonData.Instance.GetGiftItemsTypeArr(PayType.RewardsGift3Stars);
		int[] count = PayJsonData.Instance.GetGiftItemsCountsArr(PayType.RewardsGift3Stars);
