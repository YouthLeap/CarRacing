...
public class ConvertTool {
	...
	/// <summary>
	/// 字符串转为 vector2. haveBrackets = true 表示字条串包含括号
	/// </summary>
	/// <returns>The to vector2.</returns>
	/// <param name="str">String.</param>
	/// <param name="split">Split.</param>
	public static Vector2 StringToVector2(string str, char split, bool haveBrackets = true)
	{
		//去除（x^y）中的括号
		if(haveBrackets)
		{
		...
		float x = float.Parse(strTemp[0]);
		float y = float.Parse(strTemp[1]);
		Vector2 vecTemp = new Vector2(x,y);
		return vecTemp;
	}
    ...
    public static Dictionary<string, string> ToDictionary(string content, char splite, char leftTirm = (char)0x0, char rightTirm = (char)0x0)
    {
        if (leftTirm != (char)0x0)
        {
            int index = content.IndexOf(leftTirm);
            if (index != -1)
                content = content.Substring(index + 1);
        }
        if (rightTirm != (char)0x0)
        {
            int index = content.LastIndexOf(rightTirm);
            if (index != -1)
                content = content.Substring(0, index);
        }

        Dictionary<string, string> dictionary = new Dictionary<string, string>();
        string[] keyValues = content.Split(splite);
        foreach (string keyValue in keyValues)
        {
            string[] skeyValue = keyValue.Split(':');
            string key = skeyValue[0];
            string value = skeyValue[1];
